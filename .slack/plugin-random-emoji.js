...
window.slackPluginList.push({
    ...
    loadFunction: function() {
        ...
        var emojiIntervalMaxTries = 100;
        var emojiIntervalCounter = 0;
        var emojiInitializedInterval = setInterval(function() {
            try {
                emojiIntervalCounter++;
                if (emojiIntervalCounter > emojiIntervalMaxTries) {
                    clearInterval(emojiInitializedInterval);
                    return;
                }
                if (typeof window.TS.emoji === "object" && typeof window.TS.emoji.makeMenuLists === "function") {
                    initializeRandomEmoji();
                    clearInterval(emojiInitializedInterval);
                }
            } catch (exception) {
                // no-op
        ...
        function initializeRandomEmoji() {
